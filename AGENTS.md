...
- **`machine update` modifies a stopped machine.** Add/remove mounts, ports, env vars, allowed hosts (`--allow-host`/`--remove-allow-host`), or change CPU/memory without recreating the VM. Changes take effect on next `machine start`. Requires the machine to be stopped.
