...
kernel 6.8.0-1046-nvidia), and on **smolvm v1.14.2** against an **NVIDIA GeForce RTX 4050**
(Windows x86_64). Done means a program in the VM opens the device, creates a context, and moves
data to and from it.
...
> different glibc images, all three eval prompts, and the cleanup. **Windows was not re-run** and
> remains a reference page from an earlier run. The section "What was not run" lists every
> remaining step individually.
...
The Linux GPU path was re-run on v1.14.6. What remains unrun:

- **Windows**, entirely. `references/windows.md` records an earlier run on an RTX 4050 and says so.
