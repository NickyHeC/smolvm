...
**Not re-run by this packet.** Every command and every output below was executed on
Windows 11 Home build 26200 x86_64 (Intel Core Ultra 9 185H, 31.6 GiB) against smolvm v1.14.2,
and is reproduced here unchanged. Nothing on this page was run again when the packet was
written, because the Windows host was not available. Treat it as a record of one run, not as a
promise about your host.
...
extraction path that lets a failed extraction be marked complete still exists at v1.14.2
(`src/agent/manager.rs`), so the check is worth making rather than discovering afterwards.
...
```powershell
Invoke-WebRequest -Uri 'https://github.com/smol-machines/smolvm/releases/download/v1.14.2/smolvm-1.14.2-windows-x86_64.zip' -OutFile "$dir\smolvm.zip"
Invoke-WebRequest -Uri 'https://github.com/smol-machines/smolvm/releases/download/v1.14.2/checksums.sha256' -OutFile "$dir\checksums.sha256"
...
Observed: 40063732 bytes, SHA256 `771b04dda66e63d04454f7848f72d39cb9fde66e1f578330b7b313449c72e313`,
matching the release checksums exactly.

**The zip contains a nested versioned folder.** The exe is at
`dist\smolvm-1.14.2-windows-x86_64\smolvm.exe`, not `dist\smolvm.exe`. A script that assumes the
flat layout fails here, and the failure looks like a bad download.

```powershell
& "$dir\dist\smolvm-1.14.2-windows-x86_64\smolvm.exe" --version   # smolvm 1.14.2
```
...
Observed, 6.7 s including the image pull:

```
Starting ephemeral machine (vm-242bf16c)...
...
- Networking, GPU and the rest of the platform picture: see the `dev-env`, `local-api` and
  `gpu-cuda` packets. `--net` gives a real `eth0` with inbound port-forwarding, and `--cuda`
  reaches a real NVIDIA GPU. Vulkan (`--gpu`) is accepted and silently does nothing.
