...
```bash
curl -sSL https://smolmachines.com/install.sh | bash -s -- --version 1.14.2
...
- **Windows x86_64**: `references/windows.md`, and **not re-run when this packet was written**.
  Everything on that page was executed once on Windows 11 Home build 26200 against v1.14.2. The
...
- `dev-env`, `local-api`, `docker-in-machine`, `gpu-cuda` all assume the boot this packet proves.
