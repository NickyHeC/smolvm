...
this packet was written. `scripts/*.sh` are POSIX shell and do not run here.
...
A network-off run refuses to pull and prints the same hint as Unix:
...
**The `--oci-cache` bake never completes on Windows.** It stalls forever at:

```
Caching image f6d463932a2a8dda (one-time; reused on later runs)
...
nothing cleans them up; delete them like any other machine.
