...
- Counting orphans with `pgrep -f` or `readlink`
...
`readlink /proc/<pid>/exe` reports none that do: the VM process is not dumpable, so its
`/proc/<pid>/exe` is root-owned and `readlink` returns `Permission denied` to the user who started
it. A reaper built on it prints "no orphans" while an orphan runs.

`scripts/run.sh` and `scripts/cleanup.sh` read `/proc/<pid>/cmdline` and require `argv[1]` to be
exactly `_boot-vm`, which no shell can satisfy, then keep only processes whose boot config lives
under this `HOME`'s state, so another session's VM on a shared host is left alone. On macOS the
same test runs over `ps -axo pid=,command=`.
