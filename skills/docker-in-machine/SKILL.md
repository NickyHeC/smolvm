...
**This use case does not exist on Windows.** The bundled guest kernel there has neither bridge
networking nor POSIX message queues, so `dockerd` will not start and, forced past that, containers
still cannot be created. `references/windows.md` has the evidence, and the packet stops there.
...
`introduction/concepts/smolfile.md` still documents `init` as running "on every VM start". That
one wrong line is what makes the docker example look correct. More in `references/traps.md`.
...
- **Windows x86_64**: **not possible.** `references/windows.md`, from a run that was not repeated.
...
- **Windows.** One earlier run, in `references/windows.md`. The conclusion is that it cannot work,
  not that it was not tried.
