...
## What the docs get wrong for this use case, at v1.14.2
...
- `introduction/concepts/smolfile.md` documents `init` as "Commands run on every VM start". If
  that were true the example would be correct. **That single wrong line is what makes the docker
  example look right.**
