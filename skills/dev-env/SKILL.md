...
Design provisioning around the promise the docs make and the second boot comes up without it.
...
Read-only. `restart_after_stop=verified` on macOS and Linux; on Windows a stopped machine does not
start again, and the preflight says so rather than letting you find out later.
...
- **`init` runs once.** Two docs pages and `machine create --help` still say "on every VM start".
  `machine run --help` says the right thing, so the CLI now contradicts itself. Anything that must
  be true on every boot, a bind mount above all, has to run in the command that needs it.
...
- **Windows x86_64**: `references/windows.md`, **not re-run**. Create and first start match Unix;
  **a stopped machine does not start again**
  ([smolvm#1196](https://github.com/smol-machines/smolvm/issues/1196)), so the core promise of this
  use case does not hold there. That page also carries the WHP device ceiling a Windows preflight
  needs: four `-v` mounts, three when a port is published.
...
- **Windows.** Everything on `references/windows.md` is one earlier run.
- **Linux x86_64.**
