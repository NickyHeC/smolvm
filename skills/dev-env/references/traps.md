...
Two docs pages still say otherwise at v1.14.2. `introduction/concepts/smolfile.md` documents
`init` as "Commands run on every VM start", and `smolvm machine create --help` says `--init` runs
"on every VM start". `smolvm machine run --help` has the corrected wording, so the two subcommands
now contradict each other in their own help output. Believe the observed behaviour.
