...
- **Windows x86_64**: `references/windows.md`, **not re-run**. The image path works there and
  `--from-vm` has never been run on it.
...
- **Cross-platform rehydration.** Every artifact was produced and run on one architecture, and
  the two hosts used here are both arm64, so nothing tests an artifact away from the machine
  shape that made it. An arm64 artifact was never carried to an x86_64 host or the reverse.
- **`pack push`, `pack pull` and `pack inspect` against a registry.** Nothing here touched a
  registry.
- **Windows**, entirely. `references/windows.md` records an earlier run of the image path.
