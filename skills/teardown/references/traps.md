...
## `pgrep -f _boot-vm` matches your own shell, and `readlink /proc/<pid>/exe` finds nothing

Two reapers that look right and are not.

**The pattern form is too wide.** Any process whose command line contains `_boot-vm` matches,
...
**The executable form is too narrow, for a reason that is invisible.** The VM process is not
dumpable, so `/proc/<pid>/exe` is root-owned and `readlink` returns `Permission denied` to the
user who started it. Observed on Ubuntu 24.04 aarch64 on 2026-09-07 against a running machine:

```
$ ls -l /proc/51706/exe
ls: cannot read symbolic link '/proc/51706/exe': Permission denied
$ sudo readlink /proc/51706/exe
/home/<user>/skp/.smolvm/smolvm-bin
```

So a reaper built on `readlink` prints "no orphans" while an orphan runs, which is the worst of
the three outcomes.

**What works** is matching argv exactly. `scripts/cleanup.sh` reads `/proc/<pid>/cmdline`,
requires `argv[1]` to be exactly `_boot-vm`, which no shell can satisfy, and then keeps only the
processes whose boot config lives under this `HOME`'s smolvm state, so another session's VM is
left alone. On macOS the same test runs over `ps -axo pid=,command=`; there the process is
readable and `pgrep -x smolvm-bin` also finds it, but the scoping by config path still matters.
