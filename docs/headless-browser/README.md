# Headless browsers, and pre-warmed browser pools via fork

`assets/browser.smolfile` covers the basics: a GPU-accelerated headless Chromium you
`create` + `start` + `exec` against. This page covers the harder-won part:
...
The golden stays **frozen** as the CoW base while clones exist, so don't `start`
...
|---|---|
| The running browser process + warmed heap / JIT | **Live network connections** (reset on restore, so freeze the golden at an *idle* point, not mid-request) |
