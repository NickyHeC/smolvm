...
feature, not to this use case, and nothing here exercises them. There is no checkpoint route in
the spec at v1.18.2: `machine checkpoint` is CLI only, and the API restores a checkpoint through
the `from` field on create.
