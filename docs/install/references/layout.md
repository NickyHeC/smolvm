...
- `README.md` Install used to say "download from GitHub Releases, and place it into
  `~/.local/share/`". The installer places the prefix in `$HOME/.smolvm`; only the agent rootfs
  goes to a data directory. **Fixed in the commit that created this topic**: the README now names
  `~/.smolvm` and `~/.local/bin`, and `docs/install/README.md` carries the table.
- `scripts/install.sh` describes a unified `smol` CLI, and so did `README.md` at v1.14.2, though
  the README half has since gone. No v1.14.2 tarball
